...
def adapt_gcim(ham_op, ansatz_pool, HF_state, theta, 
                change_tol    = 1e-6,
                max_iter  = 200,
                file_prefix    = None,
                make_orth = False,
    ...
    for n_iter in range(0,max_iter):
        ...
        if converged:
            ...
            print(" *GCIM Finished: {:19.15f}".format(GCM_RESULT[0]) )
            gcm_diff = np.abs(true_lowest_ev-GCM_RESULT[0])
            print(" *GCIM error          {:8.6e}".format( gcm_diff ))
            # Report the converged result
            sys.stdout.flush() 
            osgcm_diffP = np.abs(true_lowest_ev-osgcm_resultP[0])
            print(" *One-shot GCIM error {:8.6e}".format( osgcm_diffP ))
        ...
        print(" GCIM Energy: {:19.15f}".format( GCM_RESULT[0]))
        print(" GCIM error : {:8.6e}".format( gcm_diff ))
